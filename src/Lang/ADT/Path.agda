...
-}
data _starts-at_ : ∀ {A} → (p : Path) → (e : ADT F A) → Set where
  tleaf : ∀ {A} {v : Variant A}
      ------------------
    → [] starts-at (leaf v)

  walk-left : ∀ {A} {D : F} {l r : ADT F A} {pl : Path}
    → pl starts-at l
      -------------------------------------
    → ((D ↣ true) ∷ pl) starts-at (D ⟨ l , r ⟩)

  walk-right : ∀ {A} {D : F} {l r : ADT F A} {pr : Path}
    → pr starts-at r
      --------------------------------------
...
-}
record PathConfig {A} (e : ADT F A) : Set where
...
-}
walk : ∀ {A} → (e : ADT F A) → PathConfig e → Variant A
...
-}
_subexprof_ : ∀ {A} → ADT F A → ADT F A → Set
