...
Complete : ∀ {V F S R} → VariabilityLanguage V F S R → Set₁
...
record TranslationResult {V F S₁ S₂ R₁ R₂} (L₁ : VariabilityLanguage V F S₁ R₁) (L₂ : VariabilityLanguage V F S₂ R₂) : Set₁ where
  field
    expr : Expression L₂ A
...
Translation : ∀ {V F S₁ S₂ R₁ R₂} (L₁ : VariabilityLanguage V F S₁ R₁) (L₂ : VariabilityLanguage V F S₂ R₂) → Set₁
...
EXPR : ∀ (V : 𝕍) (F : 𝔽) (S : 𝕊) (R : (F → S) → Set) (A : 𝔸) → VariabilityLanguage V F S R → Set
EXPR _ _ _ _ A L = Expression L A
-- EXPR : ∀ {V F S} (A : 𝔸) → VariabilityLanguage V F S → Set
-- EXPR A L = Expression L A

-- _⊆ᵥ_ : ∀ {V F S A} → IRel (EXPR V F S A) 0ℓ
_⊆ᵥ_ : ∀ {V F S R} {Γ₁ : VariabilityLanguage V F S R} {Γ₂ : VariabilityLanguage V F S R} {A}
  → Expression Γ₁ A
  → Expression Γ₂ A
  → Set
_⊆ᵥ_ {V} {_} {_} {_} {L₁} {L₂} {A} e₁ e₂ = ⟦ e₁ ⟧₁ ⊆ ⟦ e₂ ⟧₂
  where ⟦_⟧₁ = Semantics L₁
        ⟦_⟧₂ = Semantics L₂
...
_,_⊢_≚_ : ∀ {V F₁ F₂ S₁ S₂ R₁ R₂ A}
  → (Γ₁ : VariabilityLanguage V F₁ S₁ R₁)
  → (Γ₂ : VariabilityLanguage V F₂ S₂ R₂)
  → Expression Γ₁ A
  → Expression Γ₂ A
  → Set
_,_⊢_≚_ {V} {_} {_} {_} {_} {_} {_} {A} L₁ L₂ e₁ e₂ = ⟦ e₁ ⟧₁ ≅ ⟦ e₂ ⟧₂
  where ⟦_⟧₁ = Semantics L₁
        ⟦_⟧₂ = Semantics L₂
        open Data.IndexedSet (VariantSetoid V A) using (_≅_)
infix 5 _,_⊢_≚_

Conf-Preserves :  ∀ {V F S₁ S₂ R₁ R₂}
  → (L₁ : VariabilityLanguage V F S₁ R₁)
  → (L₂ : VariabilityLanguage V F S₂ R₂)
  → Expression L₁ A
  → Expression L₂ A
  → (Config F S₁ R₁ → Config F S₂ R₂)
  → Set
Conf-Preserves {F = F} {S₁ = S₁} {R₁ = R₁} L₁ L₂ e₁ e₂ conf =
  ∀ (c₁ : Config F S₁ R₁) → ⟦ e₁ ⟧₁ c₁ ≡ ⟦ e₂ ⟧₂ (conf c₁)
  where ⟦_⟧₁ = Semantics L₁
        ⟦_⟧₂ = Semantics L₂

Fnoc-Preserves :  ∀ {V F S₁ S₂ R₁ R₂}
  → (L₁ : VariabilityLanguage V F S₁ R₁)
  → (L₂ : VariabilityLanguage V F S₂ R₂)
  → Expression L₁ A
  → Expression L₂ A
  → (Config F S₂ R₂ → Config F S₁ R₁)
  → Set
Fnoc-Preserves {F = F} {S₂ = S₂} {R₂ = R₂} L₁ L₂ e₁ e₂ fnoc =
  ∀ (c₂ : Config F S₂ R₂) → ⟦ e₂ ⟧₂ c₂ ≡ ⟦ e₁ ⟧₁ (fnoc c₂)
  where ⟦_⟧₁ = Semantics L₁
        ⟦_⟧₂ = Semantics L₂

_⊆ₛ-via_ : ∀ {V F S₁ S₂ R₁ R₂} {L₁ : VariabilityLanguage V F S₁ R₁} {L₂ : VariabilityLanguage V F S₂ R₂}
  → (e : Expression L₁ A)
  → Translation L₁ L₂
  → Set
_⊆ₛ-via_ {L₁ = L₁} {L₂ = L₂} e₁ translate = Conf-Preserves L₁ L₂ e₁ (expr (translate e₁)) (conf (translate e₁))

_⊇-via_ : ∀ {V F S₁ S₂ R₁ R₂} {L₁ : VariabilityLanguage V F S₁ R₁} {L₂ : VariabilityLanguage V F S₂ R₂}
  → (e : Expression L₁ A)
  → Translation L₁ L₂
  → Set
...
_is-variant-preserving :  ∀ {V F S₁ S₂ R₁ R₂} {L₁ : VariabilityLanguage V F S₁ R₁} {L₂ : VariabilityLanguage V F S₂ R₂} → Translation L₁ L₂ → Set₁
