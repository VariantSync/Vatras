...
open import Level using (Level; _⊔_; suc; levelOfType; Setω)

private
  variable
    f s f₁ s₁ f₂ s₂ f₃ s₃ : Level
    F : 𝔽 f
    S : 𝕊 s
    F₁ : 𝔽 f₁
    F₂ : 𝔽 f₂
...
ConfigTranslation : ∀ (F₁ : 𝔽 f₁) (S₁ : 𝕊 s₁) (F₂ : 𝔽 f₂) (S₂ : 𝕊 s₂) → Set (f₁ ⊔ s₁ ⊔ f₂ ⊔ s₂)
ConfigTranslation F₁ S₁ F₂ S₂ = Config F₁ S₁ → Config F₂ S₂

record ConfigCompiler (F₁ : 𝔽 f₁) (S₁ : 𝕊 s₁) (F₂ : 𝔽 f₂) (S₂ : 𝕊 s₂) : Set (f₁ ⊔ s₁ ⊔ f₂ ⊔ s₂) where
  field
...
-}
ExtensionallyEqual-Translation : ∀ {f s} {F : 𝔽 f} {S : 𝕊 s} → ConfigTranslation F S F S → Set (f ⊔ s)
ExtensionallyEqual-Translation f = ∀ c → f c ≗ c

ExtensionallyEqual : ∀ {f s} {F : 𝔽 f} {S : 𝕊 s} → ConfigCompiler F S F S → Set (f ⊔ s)
...
Stable : ConfigCompiler F₁ S₁ F₂ S₂ → Set _
Stable cc = from cc ∘ to cc ≗ id

record LanguageCompiler {ℓ} {V : 𝕍 ℓ}
                        (Γ₁ : VariabilityLanguage V F₁ S₁)
                        (Γ₂ : VariabilityLanguage V F₂ S₂)
                        : Set (suc ℓ ⊔ levelOfType F₁ ⊔ levelOfType S₁ ⊔ levelOfType F₂ ⊔ levelOfType S₂) where
  private
    L₁ = Expression Γ₁
    L₂ = Expression Γ₂
    ⟦_⟧₁ = Semantics Γ₁
  ...
  field
    compile         : ∀ {A : 𝔸 ℓ} → L₁ A → L₂ A
    config-compiler : ConfigCompiler F₁ S₁ F₂ S₂
    preserves : ∀ {A : 𝔸 ℓ} → let open IVSet V A using (_≅[_][_]_) in
...
record ConstructCompiler {ℓ} {V : 𝕍 ℓ} (VC₁ : VariabilityConstruct V F₁ S₁) (VC₂ : VariabilityConstruct V F₂ S₂) : Set (suc ℓ ⊔ levelOfType F₁ ⊔ levelOfType S₁ ⊔ levelOfType F₂ ⊔ levelOfType S₂) where
...
-}
record ConstructFunctor {ℓ} {V : 𝕍 ℓ} (VC : VariabilityConstruct V F₁ S₁) : Setω where
  ...
  field
    map : ∀ {A} {L₁ L₂ : 𝔼 ℓ}
      ...
      → Construct F₁ L₁ A
      → Construct F₁ L₂ A
    preserves : ∀ {Γ₁ : VariabilityLanguage V F₁ S₁} {Γ₂ : VariabilityLanguage V F₂ S₂} {A}
