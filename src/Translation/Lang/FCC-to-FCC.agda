...
module IncreaseArity where
  module General where
    translate : {i : Size} → {D A : Set} → (n m : ℕ≥ 2) → n ℕ≥.≤ m → NAryChoice n D A {i} → NAryChoice m D A
    translate n m n≤m (artifact a cs) = artifact a (List.map (translate n m n≤m) cs)
    translate (sucs n) m n≤m (choice d cs) = choice d (Vec.saturate n≤m (Vec.map (translate (sucs n) m n≤m) cs))


    conf : {D : Set} → (n m : ℕ≥ 2) → n ℕ≥.≤ m → NAryChoiceConfig n D → NAryChoiceConfig m D
    conf (sucs n) (sucs m) n≤m config d = Fin.inject≤ (config d) n≤m

    fnoc : {D : Set} → (n m : ℕ≥ 2) → n ℕ≥.≤ m → NAryChoiceConfig m D → NAryChoiceConfig n D
    ...
    preserves-⊆ n m n≤m (artifact a cs) config =
      ⟦ translate n m n≤m (artifact a cs) ⟧ₙ config
      ≡⟨⟩
      ⟦ artifact a (List.map (translate n m n≤m) cs) ⟧ₙ config
      ≡⟨⟩
      ...
      ≡⟨⟩
      ⟦ artifact a cs ⟧ₙ (fnoc n m n≤m config)
      ∎
    preserves-⊆ (sucs n) (sucs m) n≤m (choice d cs) config =
      ⟦ translate (sucs n) (sucs m) n≤m (choice d cs) ⟧ₙ config
      ≡⟨⟩
      ⟦ choice d (Vec.saturate n≤m (Vec.map (translate (sucs n) (sucs m) n≤m) cs)) ⟧ₙ config
      ≡⟨⟩
      ...
      ≡⟨⟩
      ⟦ choice d cs ⟧ₙ (fnoc (sucs n) (sucs m) n≤m config)
      ∎

    preserves-⊇ : {i : Size} → {D A : Set} → (n m : ℕ≥ 2) → (n≤m : n ℕ≥.≤ m) → (expr : NAryChoice n D A {i}) → ⟦ expr ⟧ₙ ⊆[ conf n m n≤m ] ⟦ translate n m n≤m expr ⟧ₙ
    preserves-⊇ n m n≤m (artifact a cs) config =
      artifact a (List.map (λ e → ⟦ e ⟧ₙ config) cs)
      ≡⟨ Eq.cong₂ artifact Eq.refl (List.map-cong (λ e → preserves-⊇ n m n≤m e config) cs) ⟩
      artifact a (List.map (λ e → ⟦ translate n m n≤m e ⟧ₙ (conf n m n≤m config)) cs)
      ≡⟨ Eq.cong₂ artifact Eq.refl (List.map-∘ cs) ⟩
      ⟦ artifact a (List.map (translate n m n≤m) cs) ⟧ₙ (conf n m n≤m config)
      ≡⟨⟩
    ...
    preserves-⊇ (sucs n) (sucs m) n≤m (choice d cs) config =
      ...
      ≡⟨⟩
      ⟦ choice d (Vec.saturate n≤m (Vec.map (translate (sucs n) (sucs m) n≤m) cs)) ⟧ₙ (conf (sucs n) (sucs m) n≤m config)
      ≡⟨⟩
      ⟦ translate (sucs n) (sucs m) n≤m (choice d cs) ⟧ₙ (conf (sucs n) (sucs m) n≤m config)
      ∎

    preserves : {D A : Set} → (n m : ℕ≥ 2) → (n≤m : n ℕ≥.≤ m) → (expr : NAryChoice n D A) → ⟦ translate n m n≤m expr ⟧ₙ ≅[ fnoc n m n≤m ][ conf n m n≤m ] ⟦ expr ⟧ₙ
    preserves n m n≤m expr = preserves-⊆ n m n≤m expr , preserves-⊇ n m n≤m expr

  translate : {D A : Set} → (n : ℕ≥ 2) → 2AryChoice D A → NAryChoice n D A
  translate (sucs n) = General.translate (sucs zero) (sucs n) (ℕ≥.lift≤ z≤n)

  conf : {D : Set} → (n : ℕ≥ 2) → 2AryChoiceConfig D → NAryChoiceConfig n D
  conf (sucs n) = General.conf (sucs zero) (sucs n) (ℕ≥.lift≤ z≤n)

  fnoc : {D : Set} → (n : ℕ≥ 2) → NAryChoiceConfig n D → 2AryChoiceConfig D
  fnoc (sucs n) = General.fnoc (sucs zero) (sucs n) (ℕ≥.lift≤ z≤n)

  preserves : {D A : Set} → (n : ℕ≥ 2) → (expr : 2AryChoice D A) → ⟦ translate n expr ⟧ₙ ≅[ fnoc n ][ conf n ] ⟦ expr ⟧₂
  preserves (sucs n) = General.preserves (sucs zero) (sucs n) (ℕ≥.lift≤ z≤n)


module DecreaseArity where
  translate : {i : Size} → {D A : Set} → (n : ℕ≥ 2) → NAryChoice n D A {i} → 2AryChoice (D × Fin (ℕ≥.toℕ (ℕ≥.pred n))) A
  translate n (artifact a cs) = artifact a (List.map (translate n) cs)
  translate {i} {D} {A} (sucs n) (choice d cs) = go n (ℕ.n<1+n n) cs
    ...
    go zero m≤n (l ∷ r ∷ []) = choice (d , Fin.opposite (Fin.fromℕ< {zero} m≤n)) (translate (sucs n) l ∷ translate (sucs n) r ∷ [])
    go (suc m) m≤n (c ∷ cs) = choice (d , Fin.opposite (Fin.fromℕ< {suc m} m≤n)) (translate (sucs n) c ∷ go m (<-trans (ℕ.n<1+n m) m≤n) cs ∷ [])


  conf : {D : Set} → (n : ℕ≥ 2) → NAryChoiceConfig n D → 2AryChoiceConfig (D × Fin (ℕ≥.toℕ (ℕ≥.pred n)))
  conf (sucs n) config (d , m) with config d Fin.≟ (Fin.inject₁ m)
  ... | yes _ = zero
  ... | no _ = suc zero

  fnoc : {D : Set} → (n : ℕ≥ 2) → 2AryChoiceConfig (D × Fin (ℕ≥.toℕ (ℕ≥.pred n))) → NAryChoiceConfig n D
  ...
  config≡0 : {D : Set} → {d : D} → {n : ℕ} → (config : 2AryChoiceConfig (D × Fin (suc n))) → (j : Fin (suc n)) → fnoc (sucs n) config d ≡ Fin.inject₁ j → config (d , j) ≡ zero
  ...
  config≡1 : {D : Set} → {d : D} → {n : ℕ} → (config : 2AryChoiceConfig (D × Fin (suc n))) → (j : Fin (suc n)) → j Fin.< fnoc (sucs n) config d → config (d , j) ≡ suc zero
  ...
  preserves-⊆ : {i : Size} {D A : Set} → (n : ℕ≥ 2) → (expr : NAryChoice n D A {i}) → ⟦ translate n expr ⟧₂ ⊆[ fnoc n ] ⟦ expr ⟧ₙ
  preserves-⊆ (sucs n) (artifact a cs) config =
    ⟦ translate (sucs n) (artifact a cs) ⟧₂ config
    ≡⟨⟩
    ⟦ artifact a (List.map (translate (sucs n)) cs) ⟧₂ config
    ≡⟨⟩
    artifact a (List.map (λ e → ⟦ e ⟧₂ config) (List.map (translate (sucs n)) cs))
    ≡˘⟨ Eq.cong₂ artifact refl (List.map-∘ cs) ⟩
    artifact a (List.map (λ e → ⟦ translate (sucs n) e ⟧₂ config) cs)
    ≡⟨ Eq.cong₂ artifact refl (List.map-cong (λ e → preserves-⊆ (sucs n) e config) cs) ⟩
    artifact a (List.map (λ e → ⟦ e ⟧ₙ (fnoc (sucs n) config)) cs)
    ≡⟨⟩
    ⟦ artifact a cs ⟧ₙ (fnoc (sucs n) config)
    ∎
  preserves-⊆ {D = D} {A = A} (sucs n) (choice d cs) config =
    ⟦ translate (sucs n) (choice d cs) ⟧₂ config
    ...
    ≡⟨⟩
    ⟦ choice d cs ⟧ₙ (fnoc (sucs n) config)
    ∎
    where
    ...
    lemma
      ...
      → ⟦ go n d cs m m≤n cs' ⟧₂ config ≡ ⟦ Vec.lookup cs' j ⟧ₙ (fnoc (sucs n) config)
    lemma zero m≤n (l ∷ r ∷ []) zero m+config-d≡j+n =
      ⟦ go n d cs zero m≤n (l ∷ r ∷ []) ⟧₂ config
      ≡⟨⟩
      ⟦ choice (d , Fin.opposite (Fin.fromℕ< {zero} m≤n)) (translate (sucs n) l ∷ translate (sucs n) r ∷ []) ⟧₂ config
      ≡⟨⟩
      ⟦ Vec.lookup (translate (sucs n) l ∷ translate (sucs n) r ∷ []) (config (d , Fin.opposite (Fin.fromℕ< {zero} m≤n))) ⟧₂ config
      ≡⟨ Eq.cong₂ ⟦_⟧₂ (Eq.cong₂ Vec.lookup {x = translate (sucs n) l ∷ translate (sucs n) r ∷ []} refl (config≡0 config (Fin.opposite (Fin.fromℕ< m≤n)) (Fin.toℕ-injective (Eq.trans m+config-d≡j+n (Eq.trans (Eq.sym (Fin.toℕ-fromℕ n)) (Eq.trans (Eq.cong Fin.toℕ (Eq.cong Fin.opposite (Eq.sym (Fin.fromℕ<-toℕ zero m≤n)))) (Eq.sym (Fin.toℕ-inject₁ (Fin.opposite (Fin.fromℕ< m≤n)))))))))) refl ⟩
      ⟦ Vec.lookup (translate (sucs n) l ∷ translate (sucs n) r ∷ []) zero ⟧₂ config
      ≡⟨⟩
      ⟦ translate (sucs n) l ⟧₂ config
      ≡⟨ preserves-⊆ (sucs n) l config ⟩
      ⟦ l ⟧ₙ (fnoc (sucs n) config)
      ∎
    lemma zero m≤n (l ∷ r ∷ []) (suc zero) m+config-d≡j+n =
      ⟦ choice (d , Fin.opposite (Fin.fromℕ< m≤n)) (translate (sucs n) l ∷ translate (sucs n) r ∷ []) ⟧₂ config
      ≡⟨⟩
      ⟦ Vec.lookup (translate (sucs n) l ∷ translate (sucs n) r ∷ []) (config (d , Fin.opposite (Fin.fromℕ< m≤n))) ⟧₂ config
      ≡⟨ Eq.cong₂ ⟦_⟧₂ (Eq.cong₂ Vec.lookup {x = translate (sucs n) l ∷ translate (sucs n) r ∷ []} refl (config≡1 config (Fin.opposite (Fin.fromℕ< m≤n))
      ...
      ⟦ translate (sucs n) r ⟧₂ config
      ≡⟨ preserves-⊆ (sucs n) r config ⟩
      ⟦ r ⟧ₙ (fnoc (sucs n) config)
      ∎
    lemma (suc m) m≤n (c ∷ cs') zero m+config-d≡j+n =
      ⟦ choice (d , Fin.opposite (Fin.fromℕ< m≤n)) (translate (sucs n) c ∷ go n d cs m (<-trans (ℕ.n<1+n m) m≤n) cs' ∷ []) ⟧₂ config
    ...
    lemma (suc m) (s≤s m≤n) (c ∷ cs') (suc j) m+config-d≡j+n =
      ⟦ choice (d , Fin.opposite (Fin.fromℕ< (s≤s m≤n))) (translate (sucs n) c ∷ go n d cs m (<-trans (ℕ.n<1+n m) (s≤s m≤n)) cs' ∷ []) ⟧₂ config
  ...
  preserves-⊇ : {i : Size} → {D A : Set} → (n : ℕ≥ 2) → (expr : NAryChoice n D A {i}) → ⟦ expr ⟧ₙ ⊆[ conf n ] ⟦ translate n expr ⟧₂
  preserves-⊇ (sucs n) (artifact a cs) config =
    ⟦ artifact a cs ⟧ₙ config
    ≡⟨⟩
    artifact a (List.map (λ e → ⟦ e ⟧ₙ config) cs)
    ≡⟨ Eq.cong₂ artifact refl (List.map-cong (λ e → preserves-⊇ (sucs n) e config) cs) ⟩
    artifact a (List.map (λ z → ⟦ translate (sucs n) z ⟧₂ (conf (sucs n) config)) cs)
    ≡⟨ Eq.cong₂ artifact refl (List.map-∘ cs) ⟩
    artifact a (List.map (λ e → ⟦ e ⟧ₙ (conf (sucs n) config)) (List.map (translate (sucs n)) cs))
    ≡⟨⟩
    ⟦ translate (sucs n) (artifact a cs) ⟧₂ (conf (sucs n) config)
    ∎
  preserves-⊇ {D = D} {A = A} (sucs n) (choice d cs) config =
    ⟦ choice d cs ⟧ₙ config
    ≡⟨⟩
    ...
    ⟦ translate (sucs n) (choice d cs) ⟧₂ (conf (sucs n) config)
    ∎
    where
    open NAryChoice→2AryChoice-Implementation

    config≡0' : {D : Set} → {d : D} → {n : ℕ} → (config : NAryChoiceConfig (sucs n) D) → (j : Fin (suc n)) → config d ≡ (Fin.inject₁ j) → conf (sucs n) config (d , j) ≡ zero
    config≡0' {d = d} config j config-d≡j with config d Fin.≟ (Fin.inject₁ j)
    ... | yes _ = refl
    ... | no config-d≢j = ⊥-elim (config-d≢j config-d≡j)

    config≡1' : {D : Set} → {d : D} → {n : ℕ} → (config : NAryChoiceConfig (sucs n) D) → (j : Fin (suc n)) → config d ≢ (Fin.inject₁ j) → conf (sucs n) config (d , j) ≡ suc zero
    ...
    lemma
      ...
      → ⟦ go n d cs m m≤n cs' ⟧₂ (conf (sucs n) config) ≡ ⟦ Vec.lookup cs' j ⟧ₙ config
    lemma zero m≤n (l ∷ r ∷ []) zero m+config-d≡j+n =
      ⟦ go n d cs zero m≤n (l ∷ r ∷ []) ⟧₂ (conf (sucs n) config)
      ≡⟨⟩
      ⟦ choice (d , Fin.opposite (Fin.fromℕ< {zero} m≤n)) (translate (sucs n) l ∷ translate (sucs n) r ∷ []) ⟧₂ (conf (sucs n) config)
      ≡⟨⟩
      ⟦ Vec.lookup (translate (sucs n) l ∷ translate (sucs n) r ∷ []) (conf (sucs n) config (d , Fin.opposite (Fin.fromℕ< {zero} m≤n))) ⟧₂ (conf (sucs n) config)
      ≡⟨ Eq.cong₂ ⟦_⟧₂ (Eq.cong₂ Vec.lookup {x = translate (sucs n) l ∷ translate (sucs n) r ∷ []} refl (config≡0' config (Fin.opposite (Fin.fromℕ< m≤n)) (Fin.toℕ-injective (
          Fin.toℕ (config d)
          ≡⟨ m+config-d≡j+n ⟩
          n
          ≡˘⟨ Fin.toℕ-fromℕ n ⟩
      ...
      ⟦ Vec.lookup (translate (sucs n) l ∷ translate (sucs n) r ∷ []) zero ⟧₂ (conf (sucs n) config)
      ≡⟨⟩
      ⟦ translate (sucs n) l ⟧₂ (conf (sucs n) config)
      ≡˘⟨ preserves-⊇ (sucs n) l config ⟩
      ⟦ l ⟧ₙ config
      ∎
    lemma zero m≤n (l ∷ r ∷ []) (suc zero) m+config-d≡j+n =
      ⟦ choice (d , Fin.opposite (Fin.fromℕ< m≤n)) (translate (sucs n) l ∷ translate (sucs n) r ∷ []) ⟧₂ (conf (sucs n) config)
      ≡⟨⟩
      ⟦ Vec.lookup (translate (sucs n) l ∷ translate (sucs n) r ∷ []) (conf (sucs n) config (d , Fin.opposite (Fin.fromℕ< m≤n))) ⟧₂ (conf (sucs n) config)
      ≡⟨ Eq.cong₂ ⟦_⟧₂ (Eq.cong₂ Vec.lookup {x = translate (sucs n) l ∷ translate (sucs n) r ∷ []} refl (config≡1' config (Fin.opposite (Fin.fromℕ< m≤n)) (λ config-d≡opposite-m → ℕ.1+n≢n (
          suc n
          ≡˘⟨ m+config-d≡j+n ⟩
          Fin.toℕ (config d)
      ...
      ⟦ translate (sucs n) r ⟧₂ (conf (sucs n) config)
      ≡˘⟨ preserves-⊇ (sucs n) r config ⟩
      ⟦ r ⟧ₙ config
      ∎
    lemma (suc m) m≤n (c ∷ cs') zero m+config-d≡j+n =
      ⟦ choice (d , Fin.opposite (Fin.fromℕ< m≤n)) (translate (sucs n) c ∷ go n d cs m (<-trans (ℕ.n<1+n m) m≤n) cs' ∷ []) ⟧₂ (conf (sucs n) config)
      ...
      ∎
    lemma (suc m) (s≤s m≤n) (c ∷ cs') (suc j) m+config-d≡j+n =
      ⟦ choice (d , Fin.opposite (Fin.fromℕ< (s≤s m≤n))) (translate (sucs n) c ∷ go n d cs m (<-trans (ℕ.n<1+n m) (s≤s m≤n)) cs' ∷ []) ⟧₂ (conf (sucs n) config)
      ≡⟨ Eq.cong₂ ⟦_⟧ₙ (Eq.cong₂ Vec.lookup {x = translate (sucs n) c ∷ go n d cs m (<-trans (ℕ.n<1+n m) (s≤s m≤n)) cs' ∷ []} refl (config≡1' config (Fin.opposite (Fin.fromℕ< (s≤s m≤n)))
      ...
      ⟦ go n d cs m (<-trans (ℕ.n<1+n m) (s≤s m≤n)) cs' ⟧₂ (conf (sucs n) config)
      ≡⟨ lemma m (<-trans (ℕ.n<1+n m) (s≤s m≤n)) cs' j (ℕ.suc-injective m+config-d≡j+n) ⟩
      ⟦ Vec.lookup cs' j ⟧ₙ config
      ∎

  preserves : {D A : Set} → (n : ℕ≥ 2) → (expr : NAryChoice n D A) → ⟦ translate n expr ⟧₂ ≅[ fnoc n ][ conf n ] ⟦ expr ⟧ₙ
  preserves n expr = preserves-⊆ n expr , preserves-⊇ n expr


translate : {i : Size} → {D A : Set} → (n m : ℕ≥ 2) → NAryChoice n D A {i} → NAryChoice m (D × Fin (ℕ≥.toℕ (ℕ≥.pred n))) A
translate (sucs n) (sucs m) expr = IncreaseArity.translate (sucs m) (DecreaseArity.translate (sucs n) expr)

preserves : {D A : Set} → (n m : ℕ≥ 2) → (expr : NAryChoice n D A) → ⟦ translate n m expr ⟧ₙ ≅[ DecreaseArity.fnoc n ∘ IncreaseArity.fnoc m ][ IncreaseArity.conf m ∘ DecreaseArity.conf n ] ⟦ expr ⟧ₙ
