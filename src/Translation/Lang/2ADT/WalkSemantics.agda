...
-}
fun-to-path : ∀ {A} (e : 2ADT V F A) → Configuration F → PathConfig e
...
-}
path-to-fun : ∀ {A} (e : 2ADT V F A) → PathConfig e → Configuration F
...
-}
path-to-fun-lem : ∀ {A} (D : F) (e : 2ADT V F A) (p q : Path) (t : p starts-at e)
  → (b : Bool)
  → Unique p
  → p endswith ((D ↣ b) ∷ q)
