...
translate : ∀ {i : Size} {F : 𝔽} {A : 𝔸} → VariantEncoder (CCCL F) → NADT F i A → CCC F ∞ A
...
preserves-≗ : ∀ {i : Size} {F : 𝔽} {A : 𝔸} → (Variant→CCC : VariantEncoder (CCCL F)) → (expr : NADT F i A) → CCC.⟦ translate Variant→CCC expr ⟧ ≗ NADT.⟦ expr ⟧
...
preserves : ∀ {i : Size} {F : 𝔽} {A : 𝔸} → (Variant→CCC : VariantEncoder (CCCL F)) → (expr : NADT F i A) → CCC.⟦ translate Variant→CCC expr ⟧ ≅[ id ][ id ] NADT.⟦ expr ⟧
preserves Variant→CCC expr = ≗→≅[] (preserves-≗ Variant→CCC expr)

NADT→CCC : ∀ {i : Size} {F : 𝔽} → VariantEncoder (CCCL F) → LanguageCompiler (NADTL F) (CCCL F)
...
CCC≽NADT : ∀ {F : 𝔽} → VariantEncoder (CCCL F) → CCCL F ≽ NADTL F
