{-
Walk semantics are equivalent to ordinary ADT semantics
as long as the ADT does not contain any dead branches.
...
module Translation.Lang.ADT.WalkSemantics
  (F : 𝔽)
...
open import Translation.Lang.ADT.DeadElim F _==_
...
-}
fun-to-path : ∀ {A} (e : ADT F A) → Configuration F → PathConfig e
...
-}
path-to-fun : ∀ {A} (e : ADT F A) → PathConfig e → Configuration F
...
-}
path-to-fun-lem : ∀ {A} (D : F) (e : ADT F A) (p q : Path) (t : p starts-at e)
  → (b : Bool)
  → Unique p
  → p endswith ((D ↣ b) ∷ q)
...
-- we cannot construct a VariabilityLanguage for ADT with walk semantics because
