...
translate : ∀ {F : 𝔽} {A : 𝔸} → VariantEncoder (2CCL F) → ADT F A → 2CC F ∞ A
...
preserves-≗ : ∀ {F : 𝔽} {A : 𝔸} → (Variant→2CC : VariantEncoder (2CCL F)) → (expr : ADT F A) → 2CC.⟦ translate Variant→2CC expr ⟧ ≗ ADT.⟦ expr ⟧
...
preserves : ∀ {F : 𝔽} {A : 𝔸} → (Variant→2CC : VariantEncoder (2CCL F)) → (expr : ADT F A) → 2CC.⟦ translate Variant→2CC expr ⟧ ≅[ id ][ id ] ADT.⟦ expr ⟧
preserves Variant→2CC expr = ≗→≅[] (preserves-≗ Variant→2CC expr)

ADT→2CC : ∀ {F : 𝔽} → VariantEncoder (2CCL F) → LanguageCompiler (ADTL F) (2CCL F)
...
2CC≽ADT : ∀ {F : 𝔽} → VariantEncoder (2CCL F) → 2CCL F ≽ ADTL F
