...
```agda
conf : ∀ {A : 𝔸} → (e : SPL A) → FST.Configuration → VariantList.Configuration
...
```agda
fnoc : ∀ {A : 𝔸} → (e : SPL A) → VariantList.Configuration → FST.Configuration
...
preserves-⊆ : ∀ {A : 𝔸} → (e : SPL A) → VariantList.⟦ translate e ⟧ ⊆[ fnoc e ] FST.⟦ e ⟧
preserves-⊆ e@(a ◀ fs) c =
...
preserves-⊇ : ∀ {A : 𝔸} → (e : SPL A) → FST.⟦ e ⟧ ⊆[ conf e ] VariantList.⟦ translate e ⟧
preserves-⊇ e@(a ◀ fs) c =
    FST.⟦ e ⟧ c
...
preserves : ∀ {A : 𝔸} → (e : SPL A) → VariantList.⟦ translate e ⟧ ≅[ fnoc e ][ conf e ] FST.⟦ e ⟧
