...
open import Vatras.Lang.FST F as FST using (FSTL)
open FST.Impose using (SPL; Feature)
module Impose {A} = FST.Impose A
open Impose using (_◀_; _::_; name; select; forget-uniqueness; ⊛-all)

open import Vatras.Lang.VariantList (Rose ∞) as VL using (VariantList; VariantListL)
```

Modifies a configuration for one specific feature name.
```agda
config-with : Bool → F → FST.Configuration → FST.Configuration
...
```agda
configs : List F → List⁺ FST.Configuration
configs [] = (λ _ → false) ∷ []
...
```agda
translate : ∀ {A : 𝔸} → SPL A → VariantList A
translate {A} (a ◀ fs) = List⁺.map (λ c → FST.⟦ a ◀ fs ⟧ c) (configs (map name fs))

conf' : FST.Configuration → List F → ℕ
conf' c [] = 0
...
```agda
conf : ∀ {A : 𝔸} → (e : SPL A) → FST.Configuration → VL.Configuration
...
```agda
fnoc : ∀ {A : 𝔸} → (e : SPL A) → VL.Configuration → FST.Configuration
fnoc {A} (_ ◀ fs) n = find-or-last n (configs (map name fs))
```

An index computed by conf' is always in bounds
```agda
conf'<configs : ∀ (c : FST.Configuration) (fs : List F)
...
```agda
config-with-≡ : (b : Bool) → (f : F) → (c : FST.Configuration) → config-with b f c f ≡ b
...
configuration.
```agda
config-with-≢ : (b : Bool) → (f f' : F) → f ≢ f' → (c : FST.Configuration) → config-with b f' c f ≡ c f
...
```agda
find-or-last-config-with : ∀ (c : FST.Configuration) (b : Bool) (f f' : F) (fs : List F) (v : FST.Configuration → Bool)
  → ((c : FST.Configuration) → config-with b f' c f ≡ v c)
...
case `f ≢ f'`.
```agda
conf'-lemma : ∀ (c : FST.Configuration) (f : F) (fs : List F)
  → f ∈ fs
    ----------------------------------------------
...
```agda
⟦⟧-cong : ∀ {A : 𝔸} (a : atoms A) (fs : List (Feature A))
  → (c₁ c₂ : FST.Configuration)
...
preserves-⊆ : ∀ {A : 𝔸} → (e : SPL A) → VL.⟦ translate e ⟧ ⊆[ fnoc e ] FST.⟦ e ⟧
preserves-⊆ e@(a ◀ fs) c =
    VL.⟦ translate e ⟧ c
  ≡⟨⟩
...
preserves-⊇ : ∀ {A : 𝔸} → (e : SPL A) → FST.⟦ e ⟧ ⊆[ conf e ] VL.⟦ translate e ⟧
preserves-⊇ e@(a ◀ fs) c =
    FST.⟦ e ⟧ c
...
preserves : ∀ {A : 𝔸} → (e : SPL A) → VL.⟦ translate e ⟧ ≅[ fnoc e ][ conf e ] FST.⟦ e ⟧
preserves e = preserves-⊆ e , preserves-⊇ e

FST→VariantList : LanguageCompiler FSTL VariantListL
...
VariantList≽FST : VariantListL ≽ FSTL
