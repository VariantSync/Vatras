...
decideConst : ∀ p → Const p ⊎ Nonconst p
decideConst true    = left (left  λ _ → refl) -- true is const
decideConst false   = left (right λ _ → refl) -- false is const
decideConst (var x) = right ((all-true , refl) , (all-false , refl)) -- variables are unconst
decideConst (¬ p) with decideConst p -- negation is const iff inner formula is const
... | left (left  taut)  = left (right (cong Bool.not ∘ taut))
... | left (right contr) = left (left  (cong Bool.not ∘ contr))
... | right ((a , a-sat) , (b , b-unsat)) = right ((b , cong Bool.not b-unsat) , (a , cong Bool.not a-sat))
decideConst (p ∧ q) with decideConst p | decideConst q
... | left (right contr-p) | _
  -- if the left branch is a contradiction, so is the AND
  = left (right (contr-∧ˡ p q contr-p))
... | left (left p-taut)   | q-const?
  -- if the left branch is a tautology, the AND is const iff the right side is const
  = Sum.map
    (Sum.map     (taut-∧ p q p-taut) (contr-∧ʳ p q))
    (Product.map (sat-∧ˡ p q p-taut) (fal-∧    p q))
    q-const?
... | right unconst-p | left (right contr-q)
  -- if the right branch is a contradiction, so is the AND
  = left (right (contr-∧ʳ p q contr-q))
... | right unconst-p | left (left  taut-q)
  -- if the right branch is a tautology, the AND is const iff the left side is const
  = right (Product.map (sat-∧ʳ p q taut-q) (fal-∧-cong q p ∘ fal-∧ q p) unconst-p)
... | right unconst-p | right unconst-q = {!!}
